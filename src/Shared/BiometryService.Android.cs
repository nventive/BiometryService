...
{
	private const string ANDROID_KEYSTORE = "AndroidKeyStore"; // Android constant, can't be changed.
	...
	public async Task Encrypt(CancellationToken ct, string keyName, string value)
	{
		...
		var biometryCapabilites = await GetCapabilities(ct);
		if (biometryCapabilites.IsEnabled && biometryCapabilites.IsSupported)
		{
			var crypto = CreateCryptoObject(keyName);
			var authResult = await AuthenticateBiometry(ct, crypto);
			var valueToEncrypt = Encoding.UTF8.GetBytes(value);
			var encryptedData = authResult.CryptoObject.Cipher.DoFinal(valueToEncrypt);
			var initVector = authResult.CryptoObject.Cipher.GetIV();
			var bytes = new byte[initVector.Length + encryptedData.Length];

			initVector.CopyTo(bytes, 0);
			encryptedData.CopyTo(bytes, initVector.Length);
		...
		else
		{
			...
			if (biometryCapabilites.IsSupported)
			{
	...
	public async Task<string> Decrypt(CancellationToken ct, string key)
	{
		...
		var decodedData = Base64.Decode(storedData, Base64Flags.NoWrap);

		var initVector = new byte[16];
		Array.ConstrainedCopy(decodedData, 0, initVector, 0, 16);

		var buffer = new byte[decodedData.Length - 16];
		Array.ConstrainedCopy(decodedData, 16, buffer, 0, decodedData.Length - 16);

		var crypto = GetCryptoObject(key, initVector);
		var authResult = await AuthenticateBiometry(ct, crypto);
		var decryptedData = authResult.CryptoObject.Cipher.DoFinal(buffer);
